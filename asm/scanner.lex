...
"outw"     {return OUTWORD;   }
"outwh"    {return OUT6B;     }
"outd"     {return OUTDOUBLE; }
"outq"     {return OUTQUAD;   }
"syscall"  {return SYSCALL;   }
"halt"     {return SYSCALL;   }
"nop"      {return NOOP;      }
"st"	   {return STORE;     } 
"stw"	   {return STOREWORD; } 
"sth"	   {return STOREHALF; } 
"outl"	   {return OUTLOOP;   } 
"outla"	   {return OUTLOOPA;  } 
"and"	   {return AND;       } 
"andi"	   {return ANDI;      } 
"or"	   {return OR;        } 
"xor"	   {return XOR;       } 
...
\$si {
  // found a register reference
  yylval.sysword = 10; //strtol(yytext+1, NULL, 10);
  if (yylval.sysword < 0 || yylval.sysword > 31) {
    cerr << "invalid register \'"<< yytext << "\'; line " << yylineno << endl;
    exit(10);
  }
...
\$di {
  // found a register reference
  yylval.sysword = 11; //strtol(yytext+1, NULL, 10);
  if (yylval.sysword < 0 || yylval.sysword > 31) {
    cerr << "invalid register \'"<< yytext << "\'; line " << yylineno << endl;
    exit(10);
  }
...
\$dl {
  // found a register reference
  yylval.sysword = 12; //strtol(yytext+1, NULL, 10);
  if (yylval.sysword < 0 || yylval.sysword > 31) {
    cerr << "invalid register \'"<< yytext << "\'; line " << yylineno << endl;
    exit(10);
  }
...
\$do {
  // found a register reference
  yylval.sysword = 13; //strtol(yytext+1, NULL, 10);
  if (yylval.sysword < 0 || yylval.sysword > 31) {
    cerr << "invalid register \'"<< yytext << "\'; line " << yylineno << endl;
    exit(10);
  }
...
\$op {
  // found a register reference
  yylval.sysword = 29; //strtol(yytext+1, NULL, 10);
  if (yylval.sysword < 0 || yylval.sysword > 31) {
    cerr << "invalid register \'"<< yytext << "\'; line " << yylineno << endl;
    exit(10);
  }
