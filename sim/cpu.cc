...
static void print_stages(cpu_core *core)
{
	...
	printf("(ex '%s' $%-2d <= $%-2d%c($%-2d or %08x)) "
        ...
        if ((core->mys.left.control()->mem_read) && (core->mys.left.control()->mem_write)) {
		printf(" (ms '%s'  mem_mov: from 0x%08X to 0x%08X val: %08x) "
...
int run_cpu(memory *mem, const bool verbose_cpu)
{
	
	cpu_core core;

	core.PC = text_segment;
	core.IPP = data_segment;
	core.OPP = data_segment+4096;
	core.pause = false;
	core.usermode = true;
	core.mem = mem;
